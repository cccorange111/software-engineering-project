...
router.put("/updateUserInfo", async (req, res) => {
    let { account, new_account, password } = req.body;
    console.log(account, password);
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
    let { err, rows } = await db.async.all("select * from admin where account = ?", [account]);
    if (err === null && rows.length > 0) {
        let update_token_sql = "UPDATE `admin` SET `account` = ?,`password` = ? WHERE `id` = ?"
        await db.async.all(update_token_sql, [new_account, hashedPassword, rows[0].id]);
        res.send({
            code: 200,
            msg: '修改成功',
            data: {
